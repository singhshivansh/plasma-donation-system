...
def handlelogin(request):
            ...
            if user.last_name == 0:
                for i in hosuser:
                    if user.id == i.user_id:
                        ...
                        'don':don, 'hosuser': i, 'hos_feed':hos_feed, 'donor_feed':donor_feed, 'users':users})
            elif user.last_name == 1:
                 for i in hosuser:
                    if user.id == i.user_id:
                        temp = i
                        return render(request, "home/hospitalhome.html",{'obj':obj, 'status':available_, 'don1':don1,
                        'don':don, 'plasmauser': i, 'hos_feed':hos_feed, 'donor_feed':donor_feed, 'users':users})
    ...
    return render(request, "home/hospitalhome.html",{'obj':obj, 'status':available_, 'don1':don1, 'don':don,
     'hosuser':temp, 'donor_feed':donor_feed, 'hos_feed':hos_feed,'users':users, 'hosuser1':hosuser, 'plasmauser':plasmauser})
...
def hoshome(request):
    ...
    return render(request, "home/hospitalhome.html",{'obj':obj, 'don':don, 'don1':don1, 'status':available_,
    'hosuser':temp, 'hos_feed':hos_feed, 'donor_feed':donor_feed, 'users':users})
...
def viewPlasma(request, plasma_id):
    users = User.objects.all()
    hos_feed = hospital_feedback.objects.all()
    donor_feed = donor_feedback.objects.all()
    temp = users[0]
    for i in users:
        if i.id == plasma_id:
            temp = i
    return render(request, "home/view_feed.html", {"plasma_id":plasma_id, "user":temp.username, "hos_feed":hos_feed, "donor_feed":donor_feed})
